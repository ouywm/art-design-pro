...
> 后端采用 **ApiResponse** 处理成功响应 + **ProblemDetails (RFC 7807)** 处理错误响应。前端需要适配错误响应的解析方式。
...
> `type` 字段可用于区分同一 HTTP 状态码下的不同业务错误。如 401 可能是 `token-expired`（Token 过期）或 `token-invalid`（Token 无效），前端可据此做差异化处理。
